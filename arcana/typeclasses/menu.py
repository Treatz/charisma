...
def attack_node(caller):
	...
	if(caller.db.conscious == 0 and caller.db.alive == 1):
        ...
        if(init_a + 3 < init_b and caller.db.conscious and caller.db.target.db.conscious):
            ...
            text = ("|r%s is too afraid to fight" % caller.name)
...
def skip_attack(caller):
    ...
    if(caller.db.conscious == 1 or caller.db.alive == 0):
        if(int(caller.db.intimidated) == 0):
